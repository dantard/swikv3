...
from PyQt5.QtWidgets import QGraphicsRectItem, QGraphicsView, QApplication
...
class SingleColumn(Formatter):
    def format(self, page):
        y_pos = 20 if page.index == 0 else self.view.pages[page.index - 1].pos().y() + self.view.pages[page.index - 1].get_scaled_height() + page.get_sep()
...
class SingleColumnFitWidth(Formatter):
...
class LayoutManager:
    ...
    def __init__(self, view, renderer, mode=MODE_VERTICAL):
        self.view = view
        self.renderer = renderer
        self.mode = mode
        self.scene_width, self.scene_height = 0, 0
    ...
    def reset(self):
        self.scene_width, self.scene_height = 0, 0
        self.max_width, self.max_height = self.renderer.get_max_pages_size()
        self.rect = self.compute_scene_rect()
        self.view.scene().setSceneRect(self.rect)
    ...
    def single_row(self, page):
        x_pos = 20 if page.index == 0 else self.view.pages[page.index - 1].pos().x() + self.view.pages[page.index - 1].get_scaled_width() + page.get_sep()
    ...
    def vertical_multipage(self, page):
        ...
        max_num_horiz_pages = max(self.view.viewport().width() // (s_max_width + 2 * page.get_sep()), 1)
        ...
        if num_cols == 1:
            ...
            self.view.scene().setSceneRect(0, 0, self.scene_width, self.scene_height + page.get_sep())
